...
void main()
{
    ...
    vec4 texHor = texture2D(background_sampler, ShadowHorCoord);
    vec4 texVer = texture2D(background_sampler, ShadowVerCoord);
    vec4 texShadowHor = texture2D(shadow_sampler, ShadowHorCoord);
    vec4 texShadowVer = texture2D(shadow_sampler, ShadowVerCoord);
